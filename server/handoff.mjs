...
//    （トークン実値・/Users 絶対パス・Renderプロジェクト名・onrender実URL を走査する scanTextCore）
...
function scan(dir) {
  for (const e of fs.readdirSync(dir)) {
    ...
    // 共通ルール：秘密情報・絶対パス依存・Renderプロジェクト名・onrender実URL
...
// ── クリーン → zip 化（Windows/Mac 両対応・Node.js標準のみ） ──
fs.rmSync(OUT_ZIP, { force: true });

const zipSrc = path.join(STAGE, PKG_DIRNAME);
const zipEntries = [];
function walkZip(dir, base) {
  for (const name of fs.readdirSync(dir)) {
    const full = path.join(dir, name);
    const rel = base ? base + "/" + name : name;
    const st = fs.statSync(full);
    if (st.isDirectory()) { walkZip(full, rel); }
    else { zipEntries.push({ full, rel, mtime: st.mtime }); }
  }
}
walkZip(zipSrc, PKG_DIRNAME);

const zipBufs = [];
const zipCdir = [];
let zipOffset = 0;
for (const e of zipEntries) {
  const nameB = Buffer.from(e.rel, "utf8");
  const data = fs.readFileSync(e.full);
  const d = e.mtime;
  const dosT = ((d.getHours()<<11)|(d.getMinutes()<<5)|(d.getSeconds()>>1));
  const dosDt = (((d.getFullYear()-1980)<<9)|((d.getMonth()+1)<<5)|d.getDate());
  let crc = 0xFFFFFFFF;
  ...
  lh.writeUInt16LE(0,8); lh.writeUInt16LE(dosT,10); lh.writeUInt16LE(dosDt,12);
  ...
  cd.writeUInt16LE(0x800,8); cd.writeUInt16LE(0,10); cd.writeUInt16LE(dosT,12); cd.writeUInt16LE(dosDt,14);
  ...
  cd.writeUInt16LE(0,34); cd.writeUInt16LE(0,36); cd.writeUInt32LE(0,38); cd.writeUInt32LE(zipOffset,42);
  nameB.copy(cd,46);
  zipBufs.push(lh, data); zipCdir.push(cd);
  zipOffset += lh.length + data.length;
}
const cdBuf = Buffer.concat(zipCdir);
const eocd = Buffer.alloc(22);
eocd.writeUInt32LE(0x06054b50,0); eocd.writeUInt16LE(0,4); eocd.writeUInt16LE(0,6);
eocd.writeUInt16LE(zipEntries.length,8); eocd.writeUInt16LE(zipEntries.length,10);
eocd.writeUInt32LE(cdBuf.length,12); eocd.writeUInt32LE(zipOffset,16); eocd.writeUInt16LE(0,20);
fs.writeFileSync(OUT_ZIP, Buffer.concat([...zipBufs, cdBuf, eocd]));
